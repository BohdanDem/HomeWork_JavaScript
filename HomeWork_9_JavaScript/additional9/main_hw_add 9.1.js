// Є масив. Створити під кожен елемент окремий блок. В цьому блоці, під кожну властивість, та властивості внутрішніх об'єктів створити свої окремі блок.
...
for (const usersListElement of usersList) {
    ...
    user.style.background = 'lightcyan'
    ...
    name.innerText = `Name - ${usersListElement.name}`
    ...
    street.innerText = `Street - ${usersListElement.address.street}`

    let suite = document.createElement('div')
    suite.classList.add('suite')
    suite.innerText = `Suite - ${usersListElement.address.suite}`

    let city = document.createElement('div')
    city.classList.add('city')
    city.innerText = `City - ${usersListElement.address.city}`

    let zipcode = document.createElement('div')
    zipcode.classList.add('zipcode')
    zipcode.innerText = `Zipcode - ${usersListElement.address.zipcode}`

    let geo = document.createElement('div')
    geo.classList.add('geo')
    geo.innerText = 'Geo:'
    geo.style.marginLeft = '10px'

    let lat = document.createElement('div')
    lat.classList.add('lat')
    lat.innerText = `Lat - ${usersListElement.address.geo.lat}`

    let lng = document.createElement('div')
    lng.classList.add('lng')
    lng.innerText = `Lng - ${usersListElement.address.geo.lng}`
    ...
    companyName.innerText = `Name - ${usersListElement.company.name}`

    let catchPhrase = document.createElement('div')
    catchPhrase.classList.add('catch-phrase')
    catchPhrase.innerText = `CatchPhrase - ${usersListElement.company.catchPhrase}`

    let bs = document.createElement('div')
    bs.classList.add('bs')
    bs.innerText = `bs - ${usersListElement.company.bs}`

    company.append(companyName, catchPhrase, bs)
    geo.append(lat, lng)
    address.append(street, suite, city, zipcode, geo)
    user.append(id, name, username, email, address, phone, website, company)
